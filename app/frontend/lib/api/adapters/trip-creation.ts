...
export function adaptRecommendationItem(
  ...
  const photo = item.photo || item.image_snaps[0]
  return {
    id: item.id,
    name: item.name,
    match: matchPercent(item),
    category: item.primary_type_display_name || (item.categories.length > 1 ? "Mixed" : CATEGORY_LABEL[item.categories[0]]),
    subCategory: subCategory(item.categories),
    cover: apiAssetUrl(photo?.url || "/placeholder.jpg"),
    description: item.description || item.short_summary,
    estBudget: item.estimated_cost.label,
    region: item.region,
    reason: item.match_reason,
    reviewSummary: item.review_summary,
    hours: item.normalized_opening_hours || item.opening_hours_summary.summary,
    estimateNote: warningText(item),
    address: item.normalized_address || item.location.address,
    lat: item.location.lat,
    lng: item.location.lng,
    googleMapsUri: item.google_maps_uri || item.location.google_maps_uri,
    websiteUri: item.website_uri,
    rating: item.rating,
    userRatingCount: item.user_rating_count,
    primaryTypeDisplayName: item.primary_type_display_name,
